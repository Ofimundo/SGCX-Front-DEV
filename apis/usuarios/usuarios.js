...
export async function notificacionesUsuario(token) {
  try {
    const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/notificaciones`;
    const params = {
      method: "GET",
      headers: {
        "Access-Control-Allow-Origin": "*",
...
export async function notificacionesActualizar(id, token) {
  try {
    const url = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/notificaciones`;
    const params = {
      method: "PATCH",
